...
- (id)initWithFrame:(CGRect)frame BackgroundImage:(UIImage *)backgroundImage blurredImage:(UIImage *)blurredImage viewDistanceFromBottom:(CGFloat)viewDistanceFromBottom foregroundView:(UIView *)foregroundView navigationbar:(UINavigationBar*)navigationBar{
    self = [super initWithFrame:frame];
    if (self) {
        //initialize values
        _backgroundImage = backgroundImage;
        ...
        //create views
        [self createBackgroundView];
        [self createForegroundViewAndNavigationBar];
        [self createTopShadow];
        [self createBottomShadow];
    }
    return self;
...
- (void)createForegroundViewAndNavigationBar{
    
    _foregroundContainerView = [[UIView alloc] initWithFrame:self.frame];
    [self addSubview:_foregroundContainerView];
    [_navigationBar setFrame:CGRectMake(0, 0, _navigationBar.frame.size.width, DEFAULT_NAVIGATION_BAR_HEIGHT)];
    ...
    [_foregroundView setFrame:CGRectOffset(_foregroundView.bounds, (_foregroundScrollView.frame.size.width - _foregroundView.bounds.size.width)/2, _foregroundScrollView.frame.size.height - _viewDistanceFromBottom + 100)];
    [_foregroundScrollView addSubview:_foregroundView];
...
- (void)addPullToRefresh{
    __weak BTGlassScrollView *weekSelf = self;
    __weak UIScrollView *weekForegroundScrollView = _foregroundScrollView;
    
    [_foregroundScrollView addPullToRefreshWithActionHandler:^{
        [weekSelf.delegate dealWithPullToRefresh:weekForegroundScrollView];
    }];
    
    [_foregroundScrollView.pullToRefreshView setTitle:@"向下滑动刷新" forState:SVPullToRefreshStateStopped];
    [_foregroundScrollView.pullToRefreshView setTitle:@"加载中......" forState:SVPullToRefreshStateLoading];
    [_foregroundScrollView.pullToRefreshView setTitle:@"松手刷新" forState:SVPullToRefreshStateTriggered];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    // reposition the navigation bar when move over top
    [_navigationBar setFrame:CGRectMake(0, _foregroundScrollView.contentOffset.y < 0 ? - _foregroundScrollView.contentOffset.y : 0, _foregroundContainerView.frame.size.width, DEFAULT_NAVIGATION_BAR_HEIGHT)];
    // add mask for navigation bar, but not assign contentInset for _foregroundScrollView
    if (_foregroundScrollView.contentOffset.y > _foregroundView.frame.origin.y) {
        if (!_foregroundContainerView.layer.mask) {
            _foregroundContainerView.layer.mask = [self createTopMaskWithSize:CGSizeMake(_foregroundContainerView.frame.size.width, _foregroundContainerView.frame.size.height) startFadeAt:DEFAULT_NAVIGATION_BAR_HEIGHT - DEFAULT_TOP_FADING_HEIGHT_HALF endAt:DEFAULT_NAVIGATION_BAR_HEIGHT + DEFAULT_TOP_FADING_HEIGHT_HALF topColor:[UIColor colorWithWhite:1.0 alpha:0.0] botColor:[UIColor colorWithWhite:1.0 alpha:1.0]];
        }
    } else if (_foregroundContainerView.layer.mask) {
        _foregroundContainerView.layer.mask = nil;
    }
}
